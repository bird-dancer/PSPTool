...
def main():
    ...
    parser = ObligingArgumentParser(
        description="Display, extract, and manipulate AMD PSP firmware inside BIOS ROMs.\n",
    ...
    if args.extract_file:
        ...
        if args.type_regex:
            ...
            if not matching_files:
                ...
                if are_identical:
                    ph.print_warning(
                        f'Found {len(matching_files)} files matching "{args.type_regex}" but they are identical. Extracting one of them.'
                    )
                    file = matching_files[0]
                else:
                    ph.print_error_and_exit(
                        f'Found {len(matching_files)} files matching "{args.type_regex}" but they have different contents. Not extracting any.'
                    )
            else:
                file = matching_files[0]
