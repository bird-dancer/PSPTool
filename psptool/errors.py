...
class SignatureInconsistent(SignatureInvalid):
    def __str__(self):
        return (
            f"Signature for {self.signed_entity} was verified successfully at least once, but could not be "
...
class NonUniqueSignedEntity(Exception):
    ...
    def __str__(self):
        ...
        return (
            f"There was already a SignedEntity at {hex(start)}:{hex(end)} (existing={self.existing}, new={self.new})!"
        )
